...
class SinglemodalTransformer(nn.Module):
    def __init__(self, device, modality: Modality,
        ...
        # Calculate the actual input dimension after positional encoding concatenation
        # Based on the error showing 26 extra dimensions added (410-384=26)
        pos_encoding_extra_dims = 26  # Based on observed error: 410-384=26
        total_input_dim = self.modality.input_dim + pos_encoding_extra_dims
        self.input_norm = nn.LayerNorm(total_input_dim)
        self.input_proj = nn.Linear(total_input_dim, self.hidden_size)
...
class MultimodalTransformer(nn.Module):
    def __init__(self, device, modalities: Dict[str, Modality], hidden_size, latent_length, n_heads = 8, n_blocks=4,
        ...
        input_projector = {}
        for kk in self.modalities:
            modal = self.modalities[kk]
            # Calculate the actual input dimension after positional encoding concatenation
            # Based on the error showing 26 extra dimensions added (410-384=26)
            pos_encoding_extra_dims = 26  # Based on observed error: 410-384=26
            total_input_dim = modal.input_dim + pos_encoding_extra_dims
            input_projector[kk] = nn.ModuleDict(
                {
                    'input_norm': nn.LayerNorm(total_input_dim),
                    'input_proj': nn.Linear(total_input_dim, self.hidden_size)
                }
            )
...
class MultimodalTransformerWF(nn.Module):
    def __init__(self, device, modalities: Dict[str, Modality], hidden_size, latent_length, n_heads = 8, n_blocks=4,
        ...
        for kk in self.modalities:
            modal = self.modalities[kk]
            # Calculate the actual input dimension after positional encoding concatenation
            # The fourier encoding adds (2 * freq_bands + 1) dimensions per position encoding dimension
            # From the error (410 vs 384), we know 26 extra dimensions are added
            # This suggests 2 spatial dimensions * (2*freq_bands + 1 per dim) = 2 * 13 = 26
            # So each spatial dimension adds (2*freq_bands + 1) = (2*6 + 1) = 13 dimensions
            # But this is per spatial location, so if the input has spatial structure
            # we need to account for the total positional encoding size
            # Based on the error showing 26 extra dimensions added, we'll use that knowledge
            pos_encoding_extra_dims = 26  # Based on observed error: 410-384=26
            total_input_dim = modal.input_dim + pos_encoding_extra_dims
            input_projector[kk] = nn.ModuleDict(
                {
