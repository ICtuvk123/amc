...
from dataset.adni_dataset import get_dataset, ADNIDataset
...
def prediction(model, 
    ...
    for it, dbatch in pbar:
        ...
        with torch.set_grad_enabled(training):
            ...
            # flops, params = profile(model, inputs=(x,training,))
            # print('GFLOP:', flops/1e9)
            # continue
            
            loss = loss_fn(logits.squeeze(), y) / accumulation_steps
            loss += sum([loss_fn(mspc_x[mk].squeeze(), y) for mk in mspc_x]) / accumulation_steps
            losses.append(loss.item())
            if logger:
                logger.add_scalar_auto('Training Loss', loss.item())
...
def main(args):
    ...
    # ytrain = np.array(train_dataloader.dataset.all_labels)
    # weights = compute_class_weight(class_weight="balanced", classes=np.unique(np.sort(ytrain)), y=ytrain)
    # print(weights)
